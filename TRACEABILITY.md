...
## VALIDATION-001 — Trading Strategy Baseline (Trilha 2, primeiro documento)

`Docs/06-validation/VALIDATION-001-Trading-Strategy-Baseline.md` define a estratégia a validar — a própria Legacy Baseline V1 (`MarketService` → `MarketAssessmentService` → `SignalBuilderService`), já que nenhum componente do modelo-alvo (Opportunity/Decision/Market Context formal) está implementado. Referencia os critérios de entrada já existentes no código (BUY/SELL exige 4 confirmações + HIGH confidence; WAIT exige bias + ≥2 confirmações), sem alterá-los. Propõe parâmetros de validação (ativos EURUSD/USDJPY como primeira rodada, GBPUSD/XAUUSD como segunda; timeframes H4/H1/M15 já usados internamente; expectativa mínima Profit Factor > 1,0; drawdown máximo como critério de aceite da validação) — todos marcados como propostos, pendentes de confirmação do Product Owner. Mapeia diretamente ao pipeline já definido em `ADR-004` (Strategy Tester/Historical Simulation → `BACKTEST-001`; Walk Forward → `WALKFORWARD-001`; Demo Validation → `FORWARDTEST-001`).
