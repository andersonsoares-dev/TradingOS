...
| SPEC-001 | Component Model | Docs/04-specifications/SPEC-001-Component-Model.md | 1.6.0 | Approved |
...
| ROADMAP-006 | Architecture Decision Backlog | Docs/09-roadmap/ROADMAP-006-Architecture-Decision-Backlog.md | 1.4.0 | Approved |
...
- `ROADMAP-006-Architecture-Decision-Backlog.md` (solicitado originalmente como `ADB-001`, família não congelada — reprefixado para `ROADMAP-*`) registra, agora (v1.4.0), 14 itens: `ITEM-01` a `ITEM-07` da análise de evolução de `DOMAIN-003` (Evidence); `ITEM-08` da resolução de `RFC-005`; `ITEM-09` a `ITEM-14` da consolidação de `AUDIT-002`. `ITEM-07` (Observation) rejeitado. `ITEM-01`/`ITEM-02` (categorias de Evidence), `ITEM-05`/`ITEM-06` (ciclo de vida/atributos de Evidence) e `ITEM-08` (Trading Risk & Exit Strategy) **Implemented** (`ADR-011`/`ADR-013` para os dois últimos). `ITEM-03`/`ITEM-04` seguem em aberto; `ITEM-13` permanece proposto para decisão do Product Owner.
